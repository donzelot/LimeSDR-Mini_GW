...
package StageR2SDF_27 is
    type self_t is record
        shr: ShiftRegister_25.self_t;
        twiddle: complex_t(1 downto -20);
        stage1_out: complex_t(1 downto -34);
        stage2_out: complex_t(1 downto -54);
        output_index: integer;
        mode_delay: boolean;
        control: integer;
        \out\: DataValid_3.self_t;
        start_counter: DownCounter_26.self_t;
    end record;
    type StageR2SDF_27_self_t_list_t is array (natural range <>) of StageR2SDF_27.self_t;
    ...
    type self_t_const is record
        ...
        TWIDDLES: Typedefs.complex_t1downto_20_list_t(0 to 31);
        IS_TRIVIAL_MULTIPLIER: boolean;
        shr: ShiftRegister_25.self_t_const;
        \out\: DataValid_3.self_t_const;
        start_counter: DownCounter_26.self_t_const;
    end record;
    type StageR2SDF_27_self_t_const_list_t_const is array (natural range <>) of StageR2SDF_27.self_t_const;
    ...
    function StageR2SDF(shr: ShiftRegister_25.self_t; twiddle: complex_t(1 downto -20); stage1_out: complex_t(1 downto -34); stage2_out: complex_t(1 downto -54); output_index: integer; mode_delay: boolean; control: integer; \out\: DataValid_3.self_t; start_counter: DownCounter_26.self_t) return self_t;
end package;

package body StageR2SDF_27 is
    ...
    begin
        ...
        if self_const.IS_NATURAL_ORDER then
            ...
            self_next.twiddle := resize(self_const.TWIDDLES(self.control and self_const.CONTROL_MASK), 0, -10, fixed_saturate, fixed_round);
        else
            self_next.control := (self.control + 1) mod (self_const.GLOBAL_FFT_SIZE);
            self_next.twiddle := resize(self_const.TWIDDLES(self.control sra (self_const.STAGE_NR + 1)), 0, -10, fixed_saturate, fixed_round);
        end if;
        mode := not (self.control and self_const.INPUT_STRIDE);
        self_next.mode_delay := mode;
        if mode then
            ShiftRegister_25.push_next(self.shr, self_next.shr, self_const.shr, inp.data);
            ShiftRegister_25.peek(self.shr, self_next.shr, self_const.shr, pyha_ret_0);
            self_next.stage1_out := resize(pyha_ret_0, 0, -17, fixed_wrap, fixed_truncate);
        else
            ShiftRegister_25.peek(self.shr, self_next.shr, self_const.shr, pyha_ret_1);
            ...
            ShiftRegister_25.push_next(self.shr, self_next.shr, self_const.shr, down);
        ...
        if self.mode_delay and not self_const.IS_TRIVIAL_MULTIPLIER then
            self_next.stage2_out := resize(self.stage1_out * self.twiddle, 0, -27, fixed_wrap, fixed_truncate);
        else
            self_next.stage2_out := resize(self.stage1_out, 0, -27, fixed_wrap, fixed_truncate);
        ...
        DownCounter_26.tick(self.start_counter, self_next.start_counter, self_const.start_counter);
        DownCounter_26.is_over(self.start_counter, self_next.start_counter, self_const.start_counter, pyha_ret_4);
        self_next.\out\.valid := pyha_ret_4;
        ret_0 := self.\out\;
        return;
    end procedure;

    function StageR2SDF(shr: ShiftRegister_25.self_t; twiddle: complex_t(1 downto -20); stage1_out: complex_t(1 downto -34); stage2_out: complex_t(1 downto -54); output_index: integer; mode_delay: boolean; control: integer; \out\: DataValid_3.self_t; start_counter: DownCounter_26.self_t) return self_t is
        -- constructor
