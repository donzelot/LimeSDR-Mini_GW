...
package body DCRemoval_0 is
    procedure main(self:in self_t; self_next:inout self_t; constant self_const: self_t_const; x: complex_t(1 downto -34); ret_0:out complex_t(1 downto -34)) is
        ...
        variable tmp: complex_t(1 downto -34);
        variable pyha_ret_0: complex_t(1 downto -34);
    begin
        -- run input signal over all the MA's
        tmp := resize(x, 0, -17, fixed_wrap, fixed_truncate);

        for \_i_\ in self.mavg'range loop
            MovingAverage_0.main(self.mavg(\_i_\), self_next.mavg(\_i_\), self_const.mavg(\_i_\), tmp, pyha_ret_0);
            tmp := resize(pyha_ret_0, 0, -17, fixed_wrap, fixed_truncate);

            -- dc-free signal
        end loop;
        self_next.y := resize(x - tmp, 0, -17, fixed_wrap, fixed_truncate);
        ret_0 := self.y;
        return;
    end procedure;
